...
module Config = (val if mem "--arch64=false" Sys.argv then
                       (module Hamt.StdConfig32)
                     else (module Hamt.StdConfig) : Hamt.CONFIG)
...
let implem, nbiter, should_test_add, should_test_find =
  let format =
    "implem:(hamt|map) nbiter:[0-9]* options:(add|find)* \
     [--arch64={true,false}]"
  in
  let error fmt =
    Printf.kfprintf
...
let rec random_input limit n acc =
  if n = 0 then acc else random_input limit (n - 1) (Random.int limit :: acc)

let rec test_add t = function
  | [] -> t
  | i :: input -> test_add (M.add i () t) input
...
let () =
  let limit =
    ...
    if float nbiter < sqrt (float max_limit) then nbiter * nbiter else max_limit
  in
  ...
  if should_test_add then
    for _ = 0 to nb_add do
      ignore (test_add hamt input)
    done;
  if should_test_find then (
